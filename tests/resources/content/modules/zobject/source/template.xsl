...
<xsl:template match='/'>
	...
	<xsl:variable name='alt_template'>
		<xsl:choose>
			<xsl:when test='($mode="edit" or $mode="create") and string-length($ZDef/render[@type="display"]/@src)!=0'><xsl:value-of select='$ZDef/render[@type="display"]/@src'/></xsl:when>
			<xsl:when test='($mode="list-edit" or $mode="list-create") and string-length($ZDef/render[@type="list"]/@src)!=0'><xsl:value-of select='$ZDef/render[@type="list"]/@src'/></xsl:when>
		</xsl:choose>
	</xsl:variable>
...
-->
	...
	<xsl:if test='number($BenchmarkTRANSFORM)>0'><xsl:value-of select='php:functionString("BenchReport",$benchstart, "zobject transform")'/></xsl:if>
</xsl:template>
...
<xsl:template match='@*'>
	<xsl:choose>
		...
		<xsl:otherwise>
			...
			<xsl:variable name='atext'><xsl:value-of select='.'/></xsl:variable>
...
<xsl:template name='field'>
	...
	<xsl:variable name='Fbfvalue'>
		<xsl:choose>
			<xsl:when test='string-length($bfvalueFormat)!=0'><xsl:value-of select='php:functionString("php_hook", $bfvalueFormat, $bfvalue)'/></xsl:when>
	...
	<xsl:variable name='fmode'><xsl:choose><xsl:when test='$fmode1="create" and $ixf=$fid and $bfvalue!=""'>display</xsl:when><xsl:otherwise><xsl:value-of select='$fmode1'/></xsl:otherwise></xsl:choose></xsl:variable>
...
-->
			...
			<xsl:variable name='newmode'>
				<xsl:choose>
					...
					<xsl:when test='string-length($fDef/@mode)!=0'><xsl:value-of select='$fDef/@mode'/></xsl:when>
					<xsl:when test='string-length(@mode)!=0'><xsl:value-of select='@mode'/></xsl:when>
...
<table class='DEBUG'>
	...
	<xsl:template name='caption'>
		...
		<xsl:variable name='specificCap'><xsl:if test='string-length($specificZN)!=0'><xsl:value-of select='$DEFS/*/zobjectdef[@name=$specificZN]/fielddefs/fielddef[@id=$fid]/@caption'/></xsl:if></xsl:variable>
		...
		<label>
			<xsl:attribute name='for'><xsl:value-of select='$fid'/></xsl:attribute>
			<xsl:choose>
				<xsl:when test='substring($mode, 1, 4)="list"'><xsl:value-of disable-output-escaping='yes' select='php:functionString("PrettyHeader", string($bCap))'/></xsl:when>
				<xsl:otherwise><xsl:value-of disable-output-escaping='yes' select='php:functionString("PrettyCaption", string($bCap))'/></xsl:otherwise>
			</xsl:choose>
		</label>
	</xsl:template>
	...
	<xsl:template name='fieldhelp'>
		...
		<div class='capex'><xsl:copy-of select='php:functionString("PrettyCaptionHelp", string($bCap))' /></div>
	</xsl:template>
...
<table class='DEBUG'>
	...
	<tr><td>rangeTo</td><td><xsl:value-of select='$rangeTo'/></td></tr>
		...
		<xsl:for-each select='$obj/row'>
			...
			<xsl:if test='number($BenchmarkROWS)>0'><xsl:value-of select='php:functionString("BenchReport", string($rowstart), "Rows")'/></xsl:if>
			</xsl:if>
		</xsl:for-each>
	</xsl:template>
	...
	<xsl:template match='tr'>
		...
		<xsl:variable name='alt_ext'><xsl:if test='(number($R) mod 2) = 0'>-alt</xsl:if></xsl:variable>
		<tr>
		<xsl:for-each select='@*'>
			<xsl:choose>
				<xsl:when test='substring($C, 1, 1)="#" and name()="class"'><xsl:attribute name='class'><xsl:copy-of select='concat(substring($C, 2), $alt_ext)'/></xsl:attribute></xsl:when>
...
[aHREF=<xsl:value-of select='$aHREF'/>]
[nHREF=<xsl:value-of select='$nHREF'/>]
-->
		<a>
			<xsl:for-each select='@*'>
				<xsl:choose>
					<xsl:when test='name()="href"'><xsl:attribute name='href'><xsl:value-of select='$nHREF'/></xsl:attribute></xsl:when>
