...
class zobject_access
	...
	static function field_access($ZName, $fid="", &$ZMode="")
		{
        php_logger::log("CALL - $ZName, $fid, $ZMode");
		$oMode = $ZMode;
//		if (!self::zobject_access($ZName, $ZMode) && $ZMode == "") return false;
		if ($ZMode == "list" || $ZMode == "list-edit")
			{
			$test = zobject::FetchObjFieldPart($ZName, $fid, "@allow-$ZMode");
			if (!self::user_can($test)) {$ZMode="";return false;}
			}
		if ($ZMode == "create" || $ZMode == "delete")
			{
			$test = zobject::FetchObjFieldPart($ZName, $fid, "@allow-$ZMode");
		...
		if ($ZMode == "edit")
			{
			$test = zobject::FetchObjFieldPart($ZName, $fid, "@allow-$ZMode");
//print "<br/>@allow-$ZMode, test=$test";
			if (!self::user_can($test)) $ZMode = "display";
			}
		if ($ZMode == "display")
			{
